...
{
    PairToken_ = iConfig.getParameter<edm::InputTag>("pairs");
...
{
    using namespace edm;
    edm::Handle<View<reco::CompositeCandidate>> leptonPair;
    iEvent.getByLabel(PairToken_, leptonPair);
...
//    std::cout << leptonPair->size();
    for (unsigned int i = 0; i <  leptonPair->size(); ++i){
    
        const reco::CompositeCandidate& pairBuf = (*leptonPair)[i];
...
//            const edm::Ref<pat::MuonCollection> mu = lP.daughter(0)->masterClone().castTo<edm::Ref<pat::MuonCollection>>();
